...
public class AddressController {
	...
	String addressPage(Model model, HttpSession session) {
		...
		model.addAttribute("address_list",addressdao.selectAllAddress(Integer.parseInt(session.getAttribute("customerid").toString())));
		return "index";
	}
			...
			try {
				if (addressdao.createAddress(address)) {
					...
					model.addAttribute("address_list",
							addressdao.selectAllAddress(Integer.parseInt(session.getAttribute("customerid").toString())));
					model.addAttribute("Error3", false);
					model.addAttribute("edit", false);
				} else {
					model.addAttribute("addressPage", true);
